...
export class RestaurantService {
  ...
  public getRestaurants(): Promise<any> {
    return this.http
      .get<Winterlicious>("./assets/data/current-licious.json")
      .toPromise()
      .then(r => {
        ...
        winterlicious.restaurants = r.restaurants.map(restaurant => {
          restaurant.googleData.reviews = restaurant.googleData.reviews.map(
            review => Object.assign(new GoogleReview(), review)
          );
          restaurant.yelpData.reviews = restaurant.yelpData.reviews
            ? restaurant.yelpData.reviews.map(review =>
                Object.assign(new YelpReview(), review)
              )
            : [];
              ...
              "@graph": [
                {
                  ...
                  image:
                    "https://www.toronto.ca/ext/edc/licious/restaurant_images/" +
                    restaurant.lic_imagelink
                }
              ]
            }
          ];
